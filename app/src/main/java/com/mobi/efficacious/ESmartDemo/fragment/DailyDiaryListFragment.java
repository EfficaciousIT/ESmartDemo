...
ListView DairyListview;
    ...
    String userid,role_id,school_id,value,intStandard_id,intDivision_id,usertype;
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(role_id.contentEquals("2")||role_id.contentEquals("1")) {
            if (!cd.isConnectingToInternet()) {
                ...
                value= getArguments().getString("value");
                DailyDiaryStudentAsync adm = new DailyDiaryStudentAsync();
                adm.execute();
            }
        }
        return myview ;
...
String Usertype;
        ...
        protected Void doInBackground(Void... params) {
            ...
            try
            {
                ...
                if(role_id.contentEquals("3"))
                {
                    request.addProperty("command", "DailyDairyTeacher");
                        ...
                        if(response!=null)
                        {
                            ...
                            for(int j=0;j<str1.getPropertyCount();j++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if(role_id.contentEquals("5")&& value.contentEquals("HomeWork")) {
                                        if(str2.getProperty("int_Approval").toString().trim().contentEquals("0"))
                                        {
                                            diary.add(Diary);
                                        }else
                                        {
    ...
    private class DailyDiaryStudentAsync extends AsyncTask<Void, Void, Void> {
        private final ProgressDialog dialog = new ProgressDialog(getActivity());
        @Override
        protected Void doInBackground(Void... params) {
            ...
            {
                SoapObject request=new SoapObject(Constants.strNAMESPACE, OPERATION_NAME);
                    request.addProperty("command", "DailyDairyStudent");
                ...
                if(!(envelope.bodyIn instanceof SoapFault))
                {
                    ...
                    if(response != null)
                    {
                        ...
                        if(response!=null)
                        {
                            ...
                            for(int j=0;j<str1.getPropertyCount();j++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if(isValidProperty(str2, "name"))
                                    {
                                        Diary.setVchStandard(str2.getProperty("name").toString().trim());
                                    }
                        ...
                        adapter = new DailyDiaryAdapter(getActivity(),diary,value,"student");
